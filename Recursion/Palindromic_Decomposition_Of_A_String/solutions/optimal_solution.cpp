...
vector<string> generate_palindromic_decompositions(string s)
{
	...
	is_palindrome[i][j] (where i <= j and 0 <= i, j < n) will contain whether s[i, j] is a palindrome or not.
	*/
	...
	decompositions_container[0, 0] -> decompositions_container[0, 1] ->  ... -> decompositions_container[0, n - 1].
	Why we are storing these values? -> To avoid recalculation. 
	*/
	vector<vector<string>> decompositions_container(n, vector<string>(0));
	// Loop to find decompositions_container[i] (i.e. all possible palindromic decompositions of s[0, i].)
	for (int i = 0; i < n; i++)
	{
		...
		*/
		for (int j = 0; j < i; j++)
		{
			...
			{
				/*
				If s[j + 1][i] is a palindromic substring, then we can join palindromic decompositions (that we have already found) of s[0, j] with it to form the palindromic decomposition of s[0, i].
				*/
				...
				{
					/*
					Here we are directly using previously calculated values, but in recursion we recalculate them. So that is why this solution is faster than recursive solution. 
					*/
					decompositions_container[i].push_back(decompositions_container[j][k] + cur_sub_str);
				}
			}
		}
	}
