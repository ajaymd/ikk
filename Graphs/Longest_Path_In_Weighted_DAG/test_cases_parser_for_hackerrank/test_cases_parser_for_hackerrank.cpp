...
void copy_input(ifstream &fin)
{
	...
	fin >> dag_nodes >> dag_edges;

	fout << dag_nodes << " " << dag_edges << endl;
	...
	{
		fin >> dag_from[i] >> dag_to[i] >> dag_weight[i];
		fout << dag_from[i] << " " << dag_to[i] << " " << dag_weight[i] << endl;
	}

	int from_node, to_node;
	fin >> from_node >> to_node;
	fout << from_node << endl << to_node << endl;
