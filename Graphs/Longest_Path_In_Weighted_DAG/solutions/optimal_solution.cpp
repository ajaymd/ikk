...
{	// Note that we are passing node_wise_edges, topological_order, active and visited by reference. Either pass by reference or use global variables.  
	...
	// This is just for our use you can ignore it. 
	active[from] = false;
	// We could have added it at the front, but adding at the front is costly in vector type container, so I have added at the end and then we will reverse this vector. 
	topological_order.push_back(from);
}

// I will quickly try to summarize how topological sorting works. In topological sorting we try to order nodes such that, nodes reachable from any ith node comes after the ith node. Suppose we have only 2 nodes 1 and 2 and one edge 1 -> 2 then toplological order will be [1 2] because 2 is reachable from node 1, hence it should come after 1. We can use dfs to find the topological order. In dfs we visit node then its children and then "come back" to that node "after" we have finished visiting all its reachable nodes. So when we have completed visiting all children nodes, then all children will have "already" finished visiting their children and will have pushed them self in topological order! So now if we add the current node then all its children will be already added! Try some examples and it will be more clear!    
vector<int> topological_sort(int dag_nodes, vector<vector<pair<int, int>>>& node_wise_edges)
{
	...
	// We have added the nodes at the end of topological_order vector that will give nodes in reverse order of topological order.
...
vector<int> find_longest_path(int dag_nodes, vector<int> dag_from, vector<int> dag_to, vector<int> dag_weight, int from_node, int to_node)
{
	...
	// Longest_path[i] contains longest path from from_node to ith node (found till now). Here not that we are using long long int! 
	vector<long long int> longest_path(dag_nodes + 1, -1);
	// Parent[i] contains node through which Longest_path[i] updated last time. Suppose our final answer is path (1 -> 5 -> 3) then Parent[3] will be 5. This is just to track the path. Using this we will reconstruct the path. 
	vector<int> parent(dag_nodes + 1, 0);
	// Don't forget this. 
	longest_path[from_node] = 0;

	// Iterate over each node in topological order and update nodes which are connected to the current node. 
	for (int i = 0; i < dag_nodes; i++)
	{
		int from = topological_order[i];
		// We initialized longest_path of node from_node as 0 and all other nodes as -1, hence update will start only at from_node. If you are confused then better to try some examples while understanding this code. 
		if (longest_path[from] >= 0)
		{
			// "each edge" goes from upper level to lower level hence once we have found our destination node, it will not get updated by any other node so we can stop updating here. This is just an optimization, if we remove this if then also code will work fine. 
			if (from == to_node)
			{
				break;
			}
...
int main()
{
	...
	while(test_cases--)
	{
		...
		vector<int> path = find_longest_path(dag_nodes, dag_from, dag_to, dag_weight, from_node, to_node);

		map<pair<int, int>, int> weight;
		for (int i = 0; i < dag_edges; i++)
	    {
	    	assert(weight.find({dag_from[i], dag_to[i]}) == weight.end());				// Ensure no multiple edges. 
