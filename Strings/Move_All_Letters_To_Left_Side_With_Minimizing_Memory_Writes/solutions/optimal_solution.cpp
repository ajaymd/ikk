	...
	for (int i = 0; i < len; i++)
	{
		...
		if (!isdigit(s[i]))
		{
			...
			*/
			...
			/* 
			At write_here position we have put a letter (if if block is executed) or letter is already present at that position (if if block is not executed), so store next letter at write_here++ position. 
			*/
			write_here++;
		}
	}
